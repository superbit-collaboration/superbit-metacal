...
from shear_plots_fillbtwn import ShearProfilePlotter
...
def compute_shear_bias(profile_tab, shear_cutoff=None):
    '''
    ...
    if (shear_cutoff == None):
        print('No maximum shear specified for alpha calc... setting to default of gtan < 0.08')
        shear_cutoff = 0.08

    print(f'Calculating alpha for gtan < {shear_cutoff}')

    is_shear_small = (profile_tab['mean_nfw_gtan'] < shear_cutoff)
    small_shear_tab = profile_tab[is_shear_small]

    try:
        T = small_shear_tab['mean_nfw_gtan']
        D = small_shear_tab['mean_gtan']
        errbar = small_shear_tab['err_gtan']
    ...
    profile_tab.meta.update({'AlphaCalcShearCutoff': shear_cutoff})
...
class Annular(object):
    ...
    def _nfw_resample_redshift(self, gal_redshifts, outdir='.', nfactor=2,
                               overwrite=False):
        '''
        Subsample theoretical galaxy redshift distribution to match redshift
        distribution of detected galaxy catalog using a sort of MC rejection
        ...
        ax.hist(nfw_tab['redshift'],bins=100,range=[gal_redshifts.min(),\
            gal_redshifts.max()],histtype='step',label='NFW resamp', density=True)
        ax.hist(gal_redshifts,bins=100,range=[gal_redshifts.min(),\
            gal_redshifts.max()],histtype='step',label='Selected galaxies', density=True)
        ax.set_xlabel('Galaxy redshift',fontsize=12)
        ax.set_ylabel('Prob. Density',fontsize=12)
        ax.legend(fontsize=12)
        fig.savefig(os.path.join(outdir,'redshift_histograms.pdf'))
    ...
    def compute_profile(self, outfile, nfw_tab=None, overwrite=False):
        ...
        shear_cutoff = self.annular_info['shear_cutoff']
        ...
        if nfw_tab is not None:
            ...
            compute_shear_bias(profile_tab=table, shear_cutoff=shear_cutoff)
    ...
    def plot_profile(self, profile_tab, plotfile, nfw_tab=None):
        ...
        plotter.plot_tan_profile(outfile=plotfile, plot_truth=plot_truth, fill_between=False, shear_cutoff=shear_cutoff)
    ...
    def run(self, outfile, plotfile, overwrite=False):
        ...
        # Also populates shear_cutoff & other stuff from self.annular_info
