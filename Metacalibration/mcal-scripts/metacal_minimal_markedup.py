...
# Eric is defining a slightly more complicated galaxy than I was, but a similar PSF (this is ideal)
gal_ideal = galsim.InclinedExponential(50*galsim.degrees,half_light_radius=1.).rotate(45*galsim.degrees)
...
# Weights, to take care of fact that you're shearing noise as well as galaxy image
psf_weight_image = np.ones_like(psf_image.array)*1e9
weight_image = np.ones_like(gal_ideal_image.array)*1e9

# Set up for metacalibration. Doing these because it's easier
...
# Making an Observation() with observed image
...
# Get metacal images, using the metacal.py "get_all_metacal" utility function
...
###
# Now do the algorithm "by hand" -- I think?
###
# Create a sheared galaxy, convolve with metacal-type PSF, get weights
psf_metacal = galsim.Gaussian(fwhm=0.5*(1+2*mcal_shear)) # Dilution of MetaCal shear

#gal_sheared = gal_ideal.shear(galsim.Shear(g1=true_shear,g2=0.0))
gal_sheared = gal_ideal.shear(galsim.Shear(g1=0.0,g2=true_shear))
...
# Draw an image for metacal PSF
psf_metacal_image = psf_metacal.drawImage(scale=0.206)
psf_weight_image = np.ones_like(psf_metacal_image.array)*1e9

# Create dummy jacobians, instantiate Observations() for sheared galaxy and its metacal-type PSF
...
#boot.get_fitter().get_result() # I don't think this needs to be here
...
# Predicted e2 shape:
e2_pred = result['noshear'][1] + true_shear * R2
print(f"Measured e2:\n  {res['g'][1]:.4f}")
print(f"Metacal predicted e2:\n  {e2_pred:.4f}")
...
# model image
jj = gal_sheared_obs.jacobian
gmix=boot.get_fitter().get_convolved_gmix()
model_im = gmix.make_image(gal_sheared_obs.image.shape, jacobian=jj)
ax2.imshow(model_im,origin='lower')

#diff
ax3.imshow((gal_sheared_obs.image - model_im),origin='lower')

fig.savefig('minimal_mcal_2.png')
