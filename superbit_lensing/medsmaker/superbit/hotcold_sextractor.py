...
class HotColdSExtractor:
    ...
    def make_coadd_catalog(self, detection_coadd_method='single_band_coadd', detection_bandpass='b'):
        '''
        Wrapper script that runs SExtractor on coadd images based on the selected detection coadd method.
        
        Arguments:
        - detection_coadd_method: Specifies which coadd method to use ('single_band_coadd', 'detection_coadd', 'dual_image_mode').
        - detection_bandpass: The bandpass used for detection (only relevant for 'dual_image_mode' and 'detection_coadd').

        # TODO: Fix dual_image mode to potentially work with Hot/Cold one day (it is automatically set to default right now)
        '''

        # Run SExtractor on single band coadd generated by SWarp
        ...
        # Run SExtractor on dedicated detection coadd from an external source
        ...
        # Run SExtractor in dual image mode
            ...
            raise ValueError("Invalid detection_coadd_method. Choose from 'single_band_coadd', 'detection_coadd', or 'dual_image_mode'.")
