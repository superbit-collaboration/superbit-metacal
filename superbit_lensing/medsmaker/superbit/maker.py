    ...
    def _write_psf_cutouts_serial(self):
        ...
        for iobj in range(nobj):
            ...
            for icut in range(ncut):
                ...
                psfim = psf_data[file_id].draw(x=col,y=row,stamp_size=20).array
    ...
    def _set_psf_layout(self):
        ...
        for iobj in range(obj_data.size):
            for icut in range(obj_data["ncutout"][iobj]):
                ...
                elif hasattr(p,"get_rec"):
                    psf_shape = p.get_rec(row, col).shape
                    cen = p.get_center(row, col)
                else:
                    psf_shape = p.draw(x=col,y=row,stamp_size=20).array.shape
                    cenpix_row = (psf_shape[0]-1)/2
                    cenpix_col = (psf_shape[1]-1)/2
                    cen = numpy.array([cenpix_row,cenpix_col])
