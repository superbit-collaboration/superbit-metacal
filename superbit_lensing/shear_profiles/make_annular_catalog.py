...
class AnnularCatalog():
    ...
    def join(self, overwrite=False):
        ...
        if (Nobjs != Nmin):
            raise ValueError(
                f'There was an error while joining the SE and mcal catalogs;' +
                f'\nlen(SE)={self.Ndet} \nlen(mcal)={self.Nmcal}' +
                f'\nlen(joined)={Nobjs}'
            )
    ...
    def compute_metacal_quantities(self):
        ...
        # Load up config values and check that they exist in the config
        # Required keys for mcal_cuts and mcal_shear
        required_mcal_cuts_keys = ['min_Tpsf', 'min_sn', 'min_T', 'max_T', 
        'max_sn', 'max_mag_aper', 'max_mag_auto', 'max_sex_flags']
        required_mcal_shear_key = 'mcal_shear'

        # Check mcal_cuts keys exist 
        qual_cuts = self.config['mcal_cuts']
        #loop though the list of keys in required_mcal_cuts_keys and grab qual_cuts not in list 
        missing_mcal_cuts_keys = [key for key in required_mcal_cuts_keys if key not in qual_cuts]
        if missing_mcal_cuts_keys:
            raise KeyError(f"Config 'mcal_cuts' is missing the following keys: {', '.join(missing_mcal_cuts_keys)}")

        # Check mcal_shear
        if required_mcal_shear_key not in self.config:
            raise KeyError(f"Config is missing '{required_mcal_shear_key}'.")
        ...
        #inclusive boundaries added in process-real-stable
        noshear_selection = mcal[
            ...
            #& (mcal['FLAGS'] <= qual_cuts['max_sex_flags']) #added in process-real-stable
        ]
