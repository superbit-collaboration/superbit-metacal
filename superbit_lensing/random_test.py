...
'''
A "random test" is like a "pipe test", but instead of validating the current
state of the pipeline code, it generates a simplistic set of images to test
the shear calibration in a simple setting. See random_test.py & the README for
instructions on how to run such a test (just change "pipe" -> "random")
'''
...
def parse_args():
    ...
    parser.add_argument('-pipe_config', type=str, default=None,
                        help='A pipeline config to use for the random test, if ' +
                        'youd rather specify everything yourself')
    parser.add_argument('-selection_config', type=str, default=None,
                        help='A selection config to use for the random test, if ' +
                        'youd rather specify everything yourself')
    ...
    group.add_argument('-path_config', type=str, default=None,
                        ...
                        'needed to run a random test')
    group.add_argument('-gs_config', type=str, default=None,
                        help='A galsim module config to use for the random' +
    ...
    parser.add_argument('-ngmix_config', type=str, default=None,
                        help='A ngmix-fit module config to use for the random' +
                        'test, if youd rather specify everything yourself')
...
def make_test_pipe_config(gs_config, select_config, outfile='random_test.yaml',
    ...
    if (overwrite is True) or (not os.path.exists(filename)):
        ...
        if not os.path.exists(nfw_file):
            raise OSError(f'Warning: random test nfw file {nfw_file} does not ' +
        ...
        test_config = {
            ...
            f'{imsim}': {
                'config_file': os.path.join(
                    utils.TEST_DIR, run_name, 'random_test_gs.yaml'
                    ),
                'run_name': run_name,
                'outdir': outdir,
                'overwrite': overwrite
...
def make_test_gs_config(path_config, outfile='random_test_gs.yaml', outdir=None,
                        overwrite=False):
    ...
    if (overwrite is True) or (not os.path.exists(filename)):
        ...
        # use REPO/configs/random_test_gs.yaml as base

        test_config = utils.read_yaml(
            os.path.join(utils.BASE_DIR, 'configs', 'random_test_gs.yaml')
            )
...
def make_test_selection_config(outfile='random_test_select.yaml',
...
def main(args):
    ...
    if fresh is True:
        outdir = os.path.join(testdir, 'random_test')
    ...
    logfile = 'random_test.log'
    logdir = os.path.join(testdir, 'random_test')
    ...
    # a record in the random_test outdir
